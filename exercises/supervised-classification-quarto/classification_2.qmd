...
a) When using the naive Bayes classifier, the features $\xv := (x_\text{Color},x_\text{Form},x_\text{Origin})$ are assumed to be conditionally independent of each other, given the category $y = k \in \{\text{yes}, \text{no}\}$, s.t.
...
::: {.panel-tabset}
### R
{{< embed _r_2.ipynb echo=true >}}
### Python
{{< embed _python_2.ipynb echo=true >}}
:::
...
b) Given are two new observations $\xv_{*1} = -10$ and $\xv_{*2} = 7$. State the prediction for QDA and explain how you arrive there.
